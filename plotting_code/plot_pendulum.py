...
plot_pendulum.py
================
Plots MaxEntIRL (q=1, base) vs MaxEntIRL+SOAR (q=4) on Pendulum-v1
...
Run from SOAR-IL root:
    python plotting_code/plot_pendulum.py
"""
...
K_VALUES   = [1, 5, 10, 20, 50]  # must match what you ran
LOGS_ROOT  = "logs"
...
fig.suptitle(f"{ENV_NAME}  -  MaxEntIRL vs MaxEntIRL+SOAR\n(mean +/- std over seeds)",
             y=1.10, fontsize=13)
plt.tight_layout()
out1 = os.path.join(OUTPUT_DIR, "pendulum_K_comparison.png")
...
fig2.suptitle(f"{ENV_NAME}  -  Effect of K (expert trajectories)\n(mean +/- std over seeds)",
              fontsize=13)
plt.tight_layout()
out2 = os.path.join(OUTPUT_DIR, "pendulum_effect_of_K.png")
...
plt.tight_layout()
out3 = os.path.join(OUTPUT_DIR, "pendulum_final_return_bar.png")
